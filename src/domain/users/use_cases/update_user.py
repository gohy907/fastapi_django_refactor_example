...
# from src.core.exceptions.exc import UserUpdatingWithoutAuth
...
class UpdateUserUseCase:
    ...
    async def execute(self, session: AsyncSession, id: uuid.UUID, current_user: UserResponse, user_update: UserUpdate) -> UserResponse:
        if current_user.id != id:
            error = UserUpdatingWithoutAuth()

            logger.error(
                f"Пользователь с id {current_user.id} попытался отредактировать пользователя с id {id}: {
                    error.get_detail()}"
            )
            raise error
        try:
            updated_user = await self._repo.update(session=session, id=id, user_update=user_update.to_internal())
        except UserAlreadyExistsException:
            logger.info(
                f"User {user_update.login} already exists, aborting creation")
            error = UserLoginIsNotUniqueException(login=user_update.login)
            logger.error(
